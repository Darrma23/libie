...
const initializeConfig = () => {
    ...
    if (ownersEnv) {
      if (ownersEnv.startsWith("[")) {
        ...
        try {
          ...
          logger.warn("Invalid OWNERS JSON format");
        }
      } else if (ownersEnv.includes(",")) {
        // Comma-separated
        owners = ownersEnv
          .split(",")
          .map(o => o.trim())
    ...
    const config = {
        ...
        watermark: Bun.env.WATERMARK || "Libie",
        author: Bun.env.AUTHOR || "Himejima",
        stickpack: Bun.env.STICKPACK || "Libie",
        stickauth: Bun.env.STICKAUTH || "© Himejima",
...
const FAILURE_MESSAGES = {
    ...
    register: {
        title: "[ACCESS DENIED]",
        body: "This feature is only for registered users. Register yourself by means of .register.",
    },
    limit: {
        title: "[ACCESS DENIED]",
