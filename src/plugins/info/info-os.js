...
let handler = async (m, { conn }) => {
    ...
    for (const file of dbFiles) {
        try {
            ...
            dbTotalSize += stat.size;
            dbFileDetails.push(`${file}: ${formatSize(stat.size)}`);
        } catch {
            dbFileDetails.push(`${file}: not found`);
        }
    }
    
    const databaseSize = formatSize(dbTotalSize);
...
handler.desc = [
  ...
  "Menyediakan detail OS, kernel, arsitektur, dan runtime",
  "Menampilkan informasi CPU beserta load average dan state",
  "Monitoring penggunaan RAM, swap, heap, dan memori proses",
  "Menampilkan informasi disk, filesystem, dan I/O",
  "Menampilkan statistik jaringan dan traffic",
  "Menampilkan informasi proses dan thread sistem",
  "Mendeteksi container atau virtual machine",
  "Menampilkan status service dan software runtime",
  "Memberikan peringatan kondisi sistem (health check)"
];
