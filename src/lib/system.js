...
/**
 * Retrieves service information for libie bot
 * @async
...
export async function getServiceInfo() {
    const serviceName = "libie";

    const systemdStatus = await safeExec("systemctl is-active libie 2>/dev/null", "inactive");

    if (systemdStatus.trim() === "active") {
        const info = await safeExec("systemctl status libie --no-pager 2>/dev/null", "");
        if (!info) {
            return {
                name: serviceName,
                type: "systemd",
    ...
    // Check for PM2
    const pm2List = await safeExec("pm2 list 2>/dev/null | grep libie", "");
    if (pm2List.trim()) {
        const pm2Info = await safeExec("pm2 jlist 2>/dev/null", "[]");
        try {
            const processes = JSON.parse(pm2Info);
            const libieProcess = processes.find(
                (p) => p.name === "libie" || p.name.includes("libie")
            );

            if (libieProcess) {
                const uptime = libieProcess.pm2_env?.pm_uptime
                    ? Date.now() - libieProcess.pm2_env.pm_uptime
                    : 0;
                return {
                    name: serviceName,
                    type: "pm2",
                    description: "Running under PM2",
                    status: libieProcess.pm2_env?.status || "unknown",
                    active: `up ${formatTime(uptime / 1000)}`,
                    memory: libieProcess.monit?.memory
                        ? formatSize(libieProcess.monit.memory)
                        : "N/A",
                    cpu: libieProcess.monit?.cpu ? `${libieProcess.monit.cpu}%` : "N/A",
                    tasks: "N/A",
                    mainPid: libieProcess.pid || "N/A",
                };
            }
        } catch {
            // JSON parse failed
