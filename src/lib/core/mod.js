...
export class mods {
    ...
    constructor(conn) {
        ...
        // expose helper ke conn (biar plugin lama aman)
        this.conn.parseMention = this.parseMention.bind(this)
    }

    /**
     * Parse @mention dari text menjadi WhatsApp JID
     * @method parseMention
     * @param {string} text
     * @returns {string[]} mentioned JIDs
     */
    parseMention(text = '') {
        if (typeof text !== 'string') return []

        return [...text.matchAll(/@(\d{5,20})/g)]
    ...
    async client(jid, content, options = {}) {
	    if (!options.mentions) {
	        ...
	        options.mentions = this.parseMention(text)
	    }
    ...
    async sendButton(jid, content = {}, options = {}) {
        ...
        if (options.mentions?.length) {
		    if (messageContent.contextInfo) {
		        messageContent.contextInfo.mentionedJid = options.mentions
		    } else {
		        messageContent.contextInfo = {
		            mentionedJid: options.mentions
		        }
