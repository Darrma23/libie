...
export function naruyaizumi(connectionOptions) {
    ...
    conn.ev.on('group-participants.update', async (update) => {
    	try {
         ...
         const metadata = chat?.metadata || await conn.groupMetadata(id).catch(() => null)
         const groupName = metadata?.subject || 'Group'
         
         // siapa pelakunya
         const actorJid = author ? conn.decodeJid(author) : null
         const actorTag = actorJid ? actorJid.split('@')[0] : 'admin'
         ...
         for (const p of participants) {
           ...
           if (action === 'add') {
             await conn.sendMessage(id, {
               text: `Selamat datang @${tag} 👋\nDi group *${groupName}*, jangan lupa intro`,
               mentions: [jid],
             })
           }
           ...
           if (action === 'promote') {
             await conn.sendMessage(id, {
               text: `@${tag} sekarang jadi admin️\nDipromote oleh @${actorTag}`,
               mentions: [jid, actorJid].filter(Boolean),
             })
           }
         
           if (action === 'demote') {
             await conn.sendMessage(id, {
               text: `Hak admin @${tag} dicabut\nOleh @${actorTag}`,
               mentions: [jid, actorJid].filter(Boolean),
             })
           }
         }
