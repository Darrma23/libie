...
const SCHEMAS = {
    user: {
        ...
        indices: [
            "CREATE INDEX IF NOT EXISTS idx_user_jid ON user(jid)"
        ]
    },
    orders: {
        primaryKey: "id",
        ...
        indices: [
            ...
            "CREATE INDEX IF NOT EXISTS idx_orders_target_id ON orders(target_id)",
    ...
    rent: {
        ...
        indices: [
            "CREATE INDEX IF NOT EXISTS idx_rent_expired ON rent(expired)"
        ]
    },
    jadibot: {
        primaryKey: "jid",
...
// ==========================================
// 1. ENSURE TABLES (buat tabel & tambah kolom)
// ==========================================
...
// ==========================================
// 2. DEKLARASI STMTS (SETELAH ensureTable)
// ==========================================
const STMTS = {
    getRow: {},
    insertRow: {},
...
// ==========================================
// 3. PREPARE STATEMENTS (SATU LOOP UNTUK SEMUA TABEL)
// ==========================================
for (const [table, schema] of Object.entries(SCHEMAS)) {
    const pk = schema.primaryKey;

    STMTS.getRow[table] = sqlite.query(
        `SELECT * FROM ${table} WHERE ${pk} = ?`
    );

    STMTS.insertRow[table] = sqlite.query(
        `INSERT OR IGNORE INTO ${table} (${pk}) VALUES (?)`
    );

    STMTS.deleteRow[table] = sqlite.query(
...
class DataWrapper {
        ...
        return new Proxy(
            {},
            {
                get: (_, key) => {
                    ...
                    // Query database
                    let row = STMTS.getRow[table].get(key);

                    // Create row if doesn't exist
                    if (!row) {
                        STMTS.insertRow[table].run(key);
                        row = STMTS.getRow[table].get(key);
                    }

                    // Create proxy for row access
                    const proxy = this._createRowProxy(table, key, row);
                ...
                has: (_, key) => {
                    ...
                    return !!STMTS.getRow[table].get(key);
                },

                deleteProperty: (_, key) => {
                    if (typeof key !== "string") return false;

                    safeRun(() => STMTS.deleteRow[table].run(key));
                    cache.delete(`${table}:${key}`);
     ...
     */
    _createRowProxy(table, key, rowData) {
        return new Proxy(rowData, {
            set: (obj, prop, value) => {
                ...
                if (stmt) {
                    safeRun(() => stmt.run(normalizedValue, key));
